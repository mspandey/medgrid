...
import { Activity, Menu, X, ChevronDown, User, Building, Heart } from 'lucide-react';

interface NavbarProps {
    user?: { name: string; role: 'user' | 'hospital' | 'ambulance' } | null;
    logout?: () => void;
}
...
const Navbar = ({ user, logout }: NavbarProps) => {
    ...
    const handleLoginClick = (role: 'user' | 'hospital' | 'ambulance') => {
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div className="flex justify-between h-20">
                    ...
                    <div className="hidden md:flex items-center space-x-8">
                        ...
                        {user && user.role === 'user' && (
                            <Link to="/patient/dashboard" className="text-gray-600 hover:text-orthoGreen font-bold">Dashboard</Link>
                        )}
                        {user && user.role === 'hospital' && (
                            <Link to="/staff" className="text-gray-600 hover:text-orthoGreen font-bold">Staff Portal</Link>
                        )}
                        {user && user.role === 'ambulance' && (
                            <Link to="/ambulance" className="text-red-600 hover:text-red-800 font-bold animate-pulse">Ambulance Console</Link>
                        )}
                                    ...
                                    {showLoginMenu && (
                                        <div className="absolute right-0 mt-2 w-64 bg-white rounded-xl shadow-xl border border-gray-100 overflow-hidden animate-in fade-in slide-in-from-top-2">
                                            ...
                                            <button
                                                ...
                                                className="w-full text-left px-4 py-3 hover:bg-gray-50 flex items-center gap-3 border-b border-gray-50"
                                            >
                                                <div className="bg-green-50 text-green-600 p-2 rounded-lg"><Building size={18} /></div>
                                                <div>
                                            ...
                                            <button
                                                onClick={() => handleLoginClick('ambulance')}
                                                className="w-full text-left px-4 py-3 hover:bg-gray-50 flex items-center gap-3 bg-red-50/30"
                                            >
                                                <div className="bg-red-50 text-red-600 p-2 rounded-lg"><Activity size={18} /></div>
                                                <div>
                                                    <div className="font-bold text-gray-800">Ambulance</div>
                        ...
                        <div className="pt-4 border-t border-gray-100">
                            {user ? (
                                <button onClick={() => {
                                    if (logout) logout();
                                    navigate('/');
                                }} className="w-full py-2 bg-red-50 text-red-600 rounded-lg font-bold text-sm">Logout</button>
                            ) : (
                                <>
